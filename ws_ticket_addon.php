...
if(!class_exists( 'CrudClass' ) ){
    ...
    class CrudClass{
        function register(){
			  ...
			  add_shortcode('allrating',array($this,'custom_shortcode_allrating'));
...
echo '';
    ...
    return __( 'Submit', 'woocommerce' ); 
		...
		function bbloomer_add_content_thankyou() {
			 ...
			 echo '<button onclick=window.location="https://webdigitalize.com/lawyerrajeshsheoran/"; type="button" class="btn btn-primary close" data-dismiss="modal">Go to home</button>';
}
...
//             echo '</div>';
			...
			woocommerce_form_field('custom_field_name2', array(
                ...
                $checkout->get_value('custom_field_name2'));
            ...
            function custom_checkout_fields_update_order_meta( $order_id ) {
				...
				update_post_meta( $order_id, 'custom_field_name2', sanitize_text_field( $_POST['custom_field_name2'] ) );
           ...
           // 		
            ...
            function wdesk_inser_data($order_id)
            {
				...
				$tid=$order->get_meta('custom_field_name2');
...
//                 'ticket_email' 	  => $email.",".'ismailkarbhari91@gmail.com',
// 				'email' 		  => $email,
...
//             
                  $table_name = $wpdb->prefix . "wsdesk_tickets";
					$sql = $wpdb->update($table_name, array(
						'ticket_email'=>$email.",".'office@lawyerrajeshsheoran.com',
						'email'       =>$email,
						'ticket_title' => $subject,
						'priority'        =>$pits,
						'status'        =>'completed',
...
//             $id = $wpdb->insert_id;
            ...
            update_post_meta( $order_id, 'ticket_id', $tid );

				$send = eh_crm_get_settingsmeta( '0', 'auto_send_creation_email');
				if ( 'enable' == $send ) {
					$repo     = new CRM_Ajax();
				  $response = $repo->eh_crm_fire_email( 'new_ticket',  $tid );
				}
...
//             'meta_value'  =>  $meta['ticket_attachment']
//         ));
// 			 $table_names = $wpdb->prefix . "wsdesk_ticketsmeta";	
// 			$resul=$wpdb->insert( $table_names, 
//             array(
//             'ticket_id'   => $id,
//             'meta_key'     => 'ticket_attachment_path',
//             'meta_value'  =>  $meta['ticket_attachment_path']
//         ));
...
//         ));
      ...
      function custom_shortcode_rating() {
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . "wsdesk_tickets";
		$table_name1 = $wpdb->prefix . "wsdesk_ticketsmeta";  
        $sql =  $wpdb->get_results( "SELECT round(AVG(rating),1) as rating,round(count(rating)) as counts from $table_name
			where ticket_id IN
			(select ticket_id from $table_name1 where meta_value='label_LL02')");
		  $sql1 =  $wpdb->get_results( "SELECT round(AVG(rating),1) as rating,round(count(rating)) as counts from $table_name
			where rating > 0 AND ticket_id IN
			(select ticket_id from $table_name1 where meta_value='label_LL02')");
			...
			foreach($sql1 as $s1)
        {
            ...
            <p>
                <div class="placeholder" style="color: lightgray;">
                    ...
                    <i class="far fa-star"></i>
                    <i class="far fa-star"></i>
                    <span class="small" data-popup-open="popup-1"><?php echo( $rating) ?> Stars  | <?php echo( $count)?> Reviews</span>
                </div>
	...
	<!--     -->
<div class="new_class">
<!-- <a href="#" class="btn" data-popup-open="popup-1">View All Ratings</a> -->
</div>
<div class="popup" data-popup="popup-1">
  <div class="popup-inner">
    ...
    global $wpdb;
		...
		DISTINCT
         rating,(SUM(rating) OVER()) As sum_rating,(SUM(rating) OVER(PARTITION BY rating)) AS total_count,
         Round((SUM(rating) OVER(PARTITION BY rating)/SUM(rating)OVER())*100) AS percentage from $table_name");
// 	  print_r($all);
...
<div class="all start">
	<span class="astar">1  <i class="fas fa-star"></i></span>

<div class="progress progress-striped">
                      <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $all[1]->percentage ?>%">
                        <span><?php echo $all[1]->percentage ?>%</span>
                      </div>
...
<div class="all start">
	<span class="astar">2  <i class="fas fa-star"></i></span>
                    <div class="progress progress-striped">
                      <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $all[2]->percentage ?>%">
                        <span><?php echo $all[2]->percentage ?>%</span>
                      </div>
                    </div>
	 </div>

<div class="all start">
	<span class="astar">3  <i class="fas fa-star"></i></span>
 <div class="progress progress-striped">
                      <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $all[3]->percentage ?>%">
                        <span><?php echo $all[3]->percentage ?>%</span>
                      </div>
                    </div>
</div>

<div class="all start">
	<span class="astar">4  <i class="fas fa-star"></i></span>
 <div class="progress progress-striped">
                      <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $all[4]->percentage ?>%">
                        <span><?php echo $all[4]->percentage ?>%</span>
                      </div>
                    </div>
</div>

<div class="all start">
	<span class="astar">5  <i class="fas fa-star"></i></span>
 <div class="progress progress-striped">
                      <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $all[5]->percentage ?>%">
                       <span><?php echo $all[5]->percentage ?>%</span>
                      </div>
                    </div>
</div>
	 </div>
    <a href="#" class="popup-close" data-popup-close="popup-1">x</a>
		...
		function custom_shortcode_allrating() {
			global $wpdb;
        $table_name = $wpdb->prefix . "wsdesk_tickets";
		$table_name1 = $wpdb->prefix . "wsdesk_ticketsmeta"; 
			 $all =  $wpdb->get_results( "SELECT
...
<?php
        ...
        function custom_shortcode_func() {
          ...
          <h5 class="heading">Services <span class="input_required">*</span></h5>
         <div class="box">
      ...
      ?>
         ...
         <a href=""  class="btn btn-primary alt wc-forward wp-element-button" id="submit_query">Submit</a>
<!--          <button id="upadte">test</button> -->
